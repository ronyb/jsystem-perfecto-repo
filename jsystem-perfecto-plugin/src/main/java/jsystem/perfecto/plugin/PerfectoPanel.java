...
public class PerfectoPanel extends javax.swing.JPanel {
				...
				if (new File(perfectoMobileBrowserJarLocation).exists()) {
					try {
						...
						if (runningOnWindowsOS()) {
							...
							// On Mac - need to add the -XstartOnFirstThread VM argument
						...
						printProcessOutput(process, "PerfectoLabBrowserProcess");
				...
				if (new File(perfectoMobileBrowserJarLocation).exists()) {
					try {
						...
						if (runningOnWindowsOS()) {
							...
							// On Mac OS - need to add the -XstartOnFirstThread VM argument
							launchCommand = "java -XstartOnFirstThread -jar " + perfectoMobileBrowserJarLocation + " dashboard";
						}

						Process process = Runtime.getRuntime().exec(launchCommand);
						printProcessOutput(process, "PerfectoDashboardBrowserProcess");
					} catch (IOException e1) {
						e1.printStackTrace();
					}
				}
	...
	private void loadPerfectoProperties() {
		...
		if (new File("perfecto/perfecto.properties").exists()) {
			...
			hostTextField.setText(prop.getProperty("perfectoHost"));
	...
	private boolean runningOnWindowsOS() {
	...
	private void savePerfectoProperties() {
		...
		try {
			...
			prop.setProperty("perfectoHost", hostTextField.getText());
